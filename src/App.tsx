...
const LoadingSpinner = ({ t }: { t: (key: string) => string }) => {
    return (
        <div className="flex flex-col items-center justify-center h-full bg-gray-900/50 text-white">
            ...
            <p className="mt-4 text-lg">{t('loading_text')}</p>
        </div>
    );
};

const ErrorDisplay = ({ message, onRetry, t }: { message: string, onRetry: () => void, t: (key: string) => string }) => {
    return (
        <div className="flex flex-col items-center justify-center h-full bg-gray-100 dark:bg-gray-900 text-center p-4">
            <h2 className="text-xl font-bold text-red-500 mb-2">{t('scan_failed_title')}</h2>
...
const App: React.FC = () => {
    ...
    try {
      ...
      if (exhibitId) {
        ...
        if (foundExhibit) {
          ...
          console.error(`Exhibit with ID "${exhibitId}" not found in mockData.`);
          setErrorMessage(t('scan_failed_message_generic'));
          setView('error');
          analyticsService.track('scan_failed', { reason: 'not_in_database' });
        }
      } else {
        setErrorMessage(t('scan_failed_message_not_identified'));
        setView('error');
  ...
  const renderView = () => {
    switch (view) {
      ...
      case 'detail':
        if (!activeExhibit) {
          return <ErrorDisplay message={t('error_missing_exhibit')} onRetry={resetToScanner} t={t} />;
        }
      ...
      case 'error':
        return <ErrorDisplay message={errorMessage} onRetry={resetToScanner} t={t} />;
      default:
        return <ScannerView onCapture={handleCapture} />;
    }
  };
